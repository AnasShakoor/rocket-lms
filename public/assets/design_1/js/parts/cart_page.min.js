...
/*!**************************************************!*\
  !*** ./resources/js/design_1/parts/cart_page.js ***!
  \**************************************************/
(function () {
  "use strict";
  ...
  $('body').on('click', '.js-cart-payment-btn', function (e) {
    ...
    var $this = $(this);
    var $form = $this.closest('form');
    var $selectedChannel = $form.find('input[name="gateway"]:checked');
    if ($selectedChannel.length) {
      $this.addClass('loadingbar').prop('disabled', true);
      showToast("success", pleaseWaitLang, transferringToLang);
      var channelName = $selectedChannel.attr('data-class');
      if (channelName === 'Razorpay') {
        ...
        $.ajax({
          ...
          error: function error(xhr, status, _error) {
            console.error('Error fetching Moyasar form data:', _error);
  ...
  $('body').on('click', '.js-validate-coupon-btn', function (e) {
    ...
    var coupon = $parent.find('input[name="coupon"]').val();
    var path = "/cart/coupon/validate";
    if (coupon) {
      var $cartSummaryCard = $('.js-cart-summary-container');
      $this.addClass('loadingbar').prop('disabled', true);
      var data = {
        coupon: coupon
      };
