<div class="d-flex-center flex-column text-center mt-22">
    ...
    <p class="mt-4 font-12 text-gray-500">{{ trans('update.share_this_post_with_others_hint') }}</p>

    <div class="d-flex-center flex-wrap gap-16 mt-14">

        <a href="{{ $post->getShareLink('whatsapp') }}" target="_blank" class="d-flex-center size-48 rounded-12 bg-whatsapp-50">
            <x-whatsapp-bol-icon class="icons text-whatsapp" width="24px" height="24px"/>
        </a>

        <a href="{{ $post->getShareLink('twitter') }}" target="_blank" class="d-flex-center size-48 rounded-12 bg-twitter-50">
            <x-twitter-bol-icon class="icons text-twitter" width="24px" height="24px"/>
        </a>

        <a href="{{ $post->getShareLink('telegram') }}" target="_blank" class="d-flex-center size-48 rounded-12 bg-telegram-50">
            <x-telegram-bol-icon class="icons text-telegram" width="24px" height="24px"/>
        </a>

        <a href="{{ $post->getShareLink('facebook') }}" target="_blank" class="d-flex-center size-48 rounded-12 bg-facebook-50">
            <x-facebook-bol-icon class="icons text-facebook" width="24px" height="24px"/>
        </a>

        <a href="{{ $post->getShareLink('linkedin') }}" target="_blank" class="d-flex-center size-48 rounded-12 bg-linkedin-50">
