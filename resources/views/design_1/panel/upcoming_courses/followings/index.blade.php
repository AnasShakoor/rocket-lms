...
@section('content')
    ...
    @if(!empty($upcomingCourses) and !$upcomingCourses->isEmpty())
        <div id="tableListContainer" class="table-responsive-lg" data-view-data-path="/panel/upcoming_courses/followings">
            <div class="js-table-body-lists row">
                @foreach($upcomingCourses as $upcomingCourseRow)
                    <div class="col-12 col-md-6 col-lg-3 mt-20">
                        @include("design_1.panel.upcoming_courses.followings.grid_card", ['upcomingCourse' => $upcomingCourseRow])
                    </div>
                @endforeach
            </div>
