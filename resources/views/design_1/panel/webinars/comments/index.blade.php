...
@section('content')
    ...
    {{-- Top Stats --}}
    @include('design_1.panel.webinars.comments.top_stats')
        ...
        <div class="bg-white pt-16 rounded-24 mt-20">
            <div class="d-flex align-items-center justify-content-between pb-16 px-16 border-bottom-gray-100">
                <div class="">
                    ...
                    <p class="font-14 text-gray-500 mt-4">{{ trans('update.view_comments_and_related_statistics') }}</p>
                </div>
            </div>

            {{-- Filters --}}
            @include('design_1.panel.webinars.comments.filters')

            {{-- List Table --}}
            <div id="tableListContainer" class="table-responsive-lg" data-view-data-path="/panel/courses/comments">
                <table class="table panel-table">
                    ...
                    @foreach($comments as $commentRow)
                        @include('design_1.panel.webinars.comments.table_items', ['comment' => $commentRow])
                    @endforeach
                    </tbody>
                </table>
