    ...
    @if(!empty($comments) and !$comments->isEmpty())
        <div class="bg-white pt-16 rounded-24">
            <div class="d-flex align-items-center justify-content-between pb-16 px-16 border-bottom-gray-100">
                <div class="">
                    <h3 class="font-16">{{ trans('panel.my_comments') }}</h3>

                </div>
            </div>

            {{-- Filters --}}
            @include('design_1.panel.store.my_comments.filters')
            ...
            <div id="tableListContainer" class="table-responsive-lg" data-view-data-path="/panel/store/products/my-comments">
                <table class="table panel-table">
                    ...
                    @foreach($comments as $commentRow)
                        @include('design_1.panel.store.my_comments.table_items', ['comment' => $commentRow])
                    @endforeach
                    </tbody>
                </table>
